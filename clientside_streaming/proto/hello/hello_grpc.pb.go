...
const (
	...
	Example_ServerReply2_FullMethodName = "/Example/ServerReply2"
)
...
type ExampleClient interface {
	...
	ServerReply2(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[HelloRequest2, HelloResponse2], error)
}
...
func (c *exampleClient) ServerReply2(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[HelloRequest2, HelloResponse2], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Example_ServiceDesc.Streams[1], Example_ServerReply2_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[HelloRequest2, HelloResponse2]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Example_ServerReply2Client = grpc.ClientStreamingClient[HelloRequest2, HelloResponse2]
...
type ExampleServer interface {
	...
	ServerReply2(grpc.ClientStreamingServer[HelloRequest2, HelloResponse2]) error
...
func (UnimplementedExampleServer) ServerReply2(grpc.ClientStreamingServer[HelloRequest2, HelloResponse2]) error {
	return status.Errorf(codes.Unimplemented, "method ServerReply2 not implemented")
}
...
func _Example_ServerReply2_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(ExampleServer).ServerReply2(&grpc.GenericServerStream[HelloRequest2, HelloResponse2]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Example_ServerReply2Server = grpc.ClientStreamingServer[HelloRequest2, HelloResponse2]
...
var Example_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "ServerReply2",
			Handler:       _Example_ServerReply2_Handler,
			ClientStreams: true,
		},
	},
